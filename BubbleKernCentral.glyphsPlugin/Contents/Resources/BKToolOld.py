...
class BubbleKernTool(SelectTool):
	...
	def infoBox(self,sender): # CALLED IF INFO BOX UI ELEMENTS ARE EDITED
		try:
			self.activeLayer.userData['BubbleKernReferL'] = self.w.group.glyphNameL.get()
			self.activeLayer.userData['BubbleKernReferR'] = self.w.group.glyphNameR.get()
	...
	def updateUI(self, theEvent): # CALLED IF ANYTHING IN THE WINDOW CHANGES
		try:
			...
			self.w.group.glyphNameL.set(self.activeLayer.userData['BubbleKernReferL'])
			self.w.group.glyphNameR.set(self.activeLayer.userData['BubbleKernReferR'])
	...
	def saveNodesToLayer(self):
		...
		# layer.userData['BubbleKernReferL'] = self.w.group.glyphNameL.get()
		# layer.userData['BubbleKernReferR'] = self.w.group.glyphNameR.get()
	...
	def loadNodesFromLayer(self, layer=None, forceLoad=True):
		try:
			...
			m = self.font.selectedFontMaster
			self.bubbles = {}
			attributes = ('BubbleKernExportL','BubbleKernReferL','BubbleKernNodesL','BubbleKernExportR','BubbleKernReferR','BubbleKernNodesR')
			...
			# LOAD EXISTING
				...
				# self.bubbles['referL'] = layer.userData['BubbleKernReferL']
				# self.bubbles['referR'] = layer.userData['BubbleKernReferR']
	...
	# layer to draw nodes
		...
		try:
			...
			drawNodesL = self.activeLayer.userData['BubbleKernReferL'] == '' and self.activeLayer.isAligned == False
			drawNodesR = self.activeLayer.userData['BubbleKernReferR'] == '' and self.activeLayer.isAligned == False
	...
	def background(self, layer):
		...
		# 			nodes = []
		# 			if side == 'nodesL' and layer.userData['BubbleKernReferL']:
		# 				# IMPORT LEFT NODES FROM REFERRED NODE
		# 				try:
		# 					sourceLayer = f.glyphs[layer.userData['BubbleKernReferL']].layers[mId]
		# 					if 'BubbleKernNodesL' in sourceLayer.userData:
		# 						nodes = [ NSPoint(n[0], n[1]) for n in sourceLayer.userData['BubbleKernNodesL'] ]
		# 				except:
		# 					pass
		# 					# print(traceback.format_exc())
		# 			elif side == 'nodesR' and layer.userData['BubbleKernReferR']:
		# 				# IMPORT RIGHT NODES FROM REFERRED NODE
		# 				try:
		# 					sourceLayer = f.glyphs[layer.userData['BubbleKernReferR']].layers[mId]
		...
		try:
			...
			if 'BubbleKernExport'+side in userData:
				if userData['BubbleKernExport'+side] == True: # IF EXPORTS
					if 'BubbleKernRefer'+side in userData:
						if userData['BubbleKernRefer'+side] == '': # IF IT DOESN'T INHERIT ANYTHING
							return True
			return False
		except:
	...
	@objc.python_method
	def validateReferGlyph(self, layer, side):
		try:
			# expect 'L' or 'R' for side
			mId = layer.associatedMasterId
			if self.activeLayer.userData['BubbleKernRefer'+side]: # IF INHERIT ENTRY EXISTS
				...
				if self.font.glyphs[gName]:                         # REFERRED GLYPH NAME IS VALID
					...
					if 'BubbleKernRefer'+side in referredLayer.userData: # CHECK NESTED INHERITANCE
	...
	def selectAll_(self, sender):
		for side in ('nodesL', 'nodesR'):
			if self.activeLayer.isAligned == False:
				if side == 'nodesL' and self.validateReferGlyph(self.activeLayer, 'L'):
					continue
				if side == 'nodesR' and self.validateReferGlyph(self.activeLayer, 'R'):
					continue
