...
- Four ways to give a side its shape: draw it by hand, type `auto` to keep it generated from the outline, type another glyph’s name to borrow that glyph’s bubble, or type `=|` to mirror the other side of the same glyph — `=|b` mirrors another glyph’s, the way Glyphs’ own metric keys read. `=b` is taken as the plain name.
- Composite glyphs: a composite takes its bubbles from its components, so Á kerns as the A and the accent it is made of.
- Auto-generate PolyKern Groups: groups the glyphs that share a side — either by measuring which sides kern alike, or by following the kerning groups the font already has — and shows the result as the glyphs themselves with the measured bubble drawn on each. A group can be another group flipped: `d`’s left is `b`’s right, and both sides of a symmetrical glyph are one wall, so a run writes `=|` where it can and draws one wall instead of two.
